...
const TopDoctors = () => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
                ...
                <div className='border border-green-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
                    ...
                    <div className='p-4'>
                        <div className={`flex items-center gap-2 text-sm text-center ${item.available ? 'text-green-500' : 'text-gray-500'}`}>
                            <p className={`w-2 h-2 ${item.available ? 'bg-green-500' : 'bg-gray-500'} rounded-full`}></p><p>{item.available ? t('topDoctors.available') : t('topDoctors.unavailable')}</p>
                        </div>
        ...
        <button onClick={()=>{navigate('/doctors'); scrollTo(0,0) }} className='bg-green-50 text-gray-600 px-12 py-3 rounded-full mt-10'>{t('topDoctors.more')}</button>
    </div>
  )
}
