...
const paymentMidtrans = async (req, res) => {
    try {
        ...
        const snapToken = transaction.token; // Ambil snap token dari transaksi
...
const midtransWebhook = async (req, res) => {
    try {
        ...
        // Proses data webhook (Anda bisa menyesuaikan ini sesuai dengan struktur notifikasi webhook dari Midtrans)
        const appointment = await appointmentModel.findOne({ 'order_id': order_id });

        if (!appointment) {
            return res.status(404).json({ success: false, message: "Appointment not found" });
        }

        // Update status pembayaran sesuai dengan transaction_status dari Midtrans
        if (transaction_status === 'settlement') {
            await appointmentModel.findByIdAndUpdate(appointment._id, { paymentStatus: 'paid' });
        } else if (transaction_status === 'cancel' || transaction_status === 'expire') {
            await appointmentModel.findByIdAndUpdate(appointment._id, { paymentStatus: 'failed' });
        }

        // Kirim respon kepada Midtrans setelah berhasil menerima notifikasi
...
export { registerUser, getProfile, loginUser, updateProfile, bookAppointment, listAppointment, cancelAppointment, paymentMidtrans, midtransWebhook, getAppointmentStatus }